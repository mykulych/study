...
export default function BlogPostsPageTest2() {
  
  const [questionsRandom, setQuestionsRandom] = useState([]);

  const [questions, setQuestions] = useState(() => {
    const storedQuestions = JSON.parse(localStorage.getItem('questions_middle')) || [];
    ...
    if (nextQuestion < questionsRandom.length) {
      ...
      const updatedUsers = users.map((user) => {
        if (
          ...
          if (!user.results_two) {
            user.results_two = []; // Ініціалізуємо results як порожній масив, якщо вона не існує
          }
          console.log(score);
          if (isCorrect){
            setScore(score+1);
            user.results_two.push(score+1);
          }else
          user.results_two.push(score);
  ...
  const handleRestartQuiz = () => {
    const allQuestions = JSON.parse(localStorage.getItem('questions_middle')) || [];
...
const handleAddQuestion = () => {
  ...
  localStorage.setItem('questions_middle', JSON.stringify(updatedQuestions));
